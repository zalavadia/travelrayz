...
/* ── Seed testimonials (run once from editor; skips duplicates) ── */

function normalizeTestimonialKey(name, text) {
  var n = String(name || "")
    .trim()
    .toLowerCase();
  var t = String(text || "")
    ...
    .trim()
    .toLowerCase();
  return n + "|" + t.slice(0, 140);
}

function testimonialExists(existing, name, text) {
  var key = normalizeTestimonialKey(name, text);
  for (var i = 0; i < existing.length; i++) {
    var row = existing[i];
    if (normalizeTestimonialKey(row.name, row.text) === key) return true;
  }
  return false;
}

function defaultTestimonialsSeed() {
  return [
    {
      name: "Uma Nagpal",
      rating: "5",
      trip: "Sondai Trek",
      text:
        "A Day to meet new friends, make wonderful memories and experience the scenic beauty of the trek. It was an awesome experience where all were in great energetic moods enjoying in a group. Had great time with you guys. Thank you for your company and making this trip a memorable one for me. Best wishes to the Team Travel Rayz - Pratima, Kiran, Rishi, Onkar. The trek leaders were very caring, friendly and on their toes with great co-ordination among themselves. For any situation, the response was immediate and acted instantly. The arrangements Well planned from food to everything and You made certain that everything goes well ensuring safety of everyone. Great Job"
    },
    {
      name: "Chitra",
      rating: "4.5",
      trip: "Aadrai Trek",
      text:
        "It was a great trip. You coordinated with everyone really well and handled everything so smoothly. Thank you for making the experience so comfortable and enjoyable! I felt very comfortable, safe and free throughout the trek, As a girl that feeling of safety means a lot."
    },
    {
      name: "Vijay Naidu",
      rating: "5",
      trip: "Sondai Trek",
      text:
        "I want to thank each one of you for the wonderful trek. Had great time with you guys. Thank you for memorable one for me. Kudos to Team TravelRayz - Pratima, Kiran, Rishi, Onkar. The trek was well planned and well coordinated You ensured safety. Good Job"
    },
    {
      name: "Deven",
      rating: "5",
      trip: "Raigad Fort",
      text:
        "Safely back home, guys! I had so much fun today. It truly didn't feel like a first meetup at all the vibe was amazing. Big thanks to Pratima and Rani! Sending lots of love to the whole group!"
    },
    {
      name: "Vishal",
      rating: "4.5",
      trip: "Devkund Waterfall",
      text:
        "Thanks Captain and TravelRayz for an amazing trip! Everyone was wonderful, made a few really good friends along the way, enjoyed every single moment. Keep traveling, and let's cross paths again on some random trip soon"
    },
    {
      name: "Aaradhana",
      rating: "4.5",
      trip: "Devkund Waterfall",
      text:
        "Thanks guys\nIt was my first trek and I had a great experience\nEveryone helped me a lot."
    },
    {
      name: "Chirag",
      rating: "5",
      ...
      text:
        "Thank you so much Captains for making this trip memorable, Had a best time with you all and lot of unforgettable memories. I had a fun. See you on the next trip."
    },
    {
      name: "Bhavesh",
      rating: "4.5",
      trip: "Kedarnath Yatra",
      text:
        "Thank you Everyone for making the Kedarnath journey/trip memorable, adventure, fun, and one to always remember in my Highlights and will miss you all for the memories we shared together."
    },
    {
      name: "Nikita",
      rating: "5",
      trip: "Kedarnath Yatra",
      text:
        "Thank you so much everyone, especially Pratima, Kiran, Akshay for the best trip management and for a lifetime memory. I'll miss y'all, we laughed, we struggled,we danced, we trekked all the way above ,and in the end it was all worth it with you guys. A lot of love to you all."
    }
  ];
}

/** Run from Apps Script editor: adds seed stories only if not already on the sheet */
function seedTestimonialsIfMissing() {
  var existing = getTestimonials(true);
  var added = 0;
  var skipped = 0;
  defaultTestimonialsSeed().forEach(function (item) {
    if (testimonialExists(existing, item.name, item.text)) {
      skipped++;
      return;
    }
    addTestimonial({
  ...
  Logger.log("Testimonials seed complete. Added: " + added + ", skipped (duplicate): " + skipped);
